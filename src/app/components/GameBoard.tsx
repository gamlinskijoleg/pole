...
export default function GameBoard({
    ...
    return (
        <div className={styles.gameBoard}>
            ...
            <div style={{ display: 'flex', gap: '10px', width: '100%', maxWidth: '500px', justifyContent: 'space-between' }}>
                <button
                    className={styles.button}
                    style={{ width: 'auto', padding: '5px 15px', fontSize: '0.9rem', background: '#666' }}
                    onClick={onToMenu}
                >
                    В меню (Пауза)
                </button>
                <button
                    className={styles.button}
                    style={{ width: 'auto', padding: '5px 15px', fontSize: '0.9rem', background: '#d32f2f' }}
                    onClick={onReset}
                >
                    Почати знову
                </button>
            ...
            <div className={styles.infoPanel}>
                <h2>
                    Хід: <span style={{ color: currentPlayer?.color }}>
                        {currentPlayer?.name}
                    </span>
                </h2>
            ...
            >
                {grid.map((cell, idx) => {
                    ...
                    // Логіка підсвітки
                    const isNeighbor = currentPlayerId !== null && isNeighborToCurrent(cell);
