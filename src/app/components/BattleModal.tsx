...
export default function BattleModal({ battleData, players, onAnswer }: BattleModalProps) {
    const attacker = players.find(p => p.id === battleData.attackerId);
    const defender = players.find(p => p.id === battleData.defenderId);

    const isPenaltyActive = !!battleData.penaltyUntil && Date.now() < battleData.penaltyUntil;
